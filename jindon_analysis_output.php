<?php
    // http://localhost/xampp/jindon_analysis_output.php
    // 將TF-IDF排序，按每50個詞彙做一級距回塞analysis_output
...
if(1){  // 清除原本analysys_output，計算TF-IDF寫回資料表
    echo "清除原本analysys_output<BR>";
    $sql = "DELETE FROM analysis_output";
    ...
    foreach ($id_array as $fan_group_name => $data)
    {
        // 將TF-IDF的結果排序
        // 排除特定詞類，長度為1的詞彙排除
        $sql = "SELECT CONCAT(WORD, '(', TYPE, ')') AS WORD_TYPE, `{$fan_group_name}`
                  FROM `message_tfidf`
        ...
        // MYSQL不支援SUBQUERY的LIMIT語法，只好拆成兩段處理
        $res = $conn->query("SELECT MSG_ID
                               FROM message_group 
                              WHERE FAN_GROUP_NAME = '{$fan_group_name}' 
                                AND CHAR_LENGTH(CKIP_WORD) > 3
                              ORDER BY SHARES_COUNT DESC LIMIT 5");
        ...
        foreach ($all_msg_array as $row)
        {
            ...
            foreach ($tfidf_array as $key => $word)
            {
                ...
                else if ($key >= 500)
                {
                    ...
                    // 每五十個字一個級距，寫入DB
                    $sql = "INSERT INTO analysis_output (FAN_GROUP_NAME, MSG_ID, LV01, LV02, LV03, LV04, LV05, LV06, LV07, LV08, LV09, LV10, SHARES_COUNT, LIKES_COUNT) VALUES
                                                        (:FAN_GROUP_NAME, :MSG_ID, :LV01, :LV02, :LV03, :LV04, :LV05, :LV06, :LV07, :LV08, :LV09, :LV10, :SHARES_COUNT, :LIKES_COUNT)";
                    $bind = array(':FAN_GROUP_NAME' => $fan_group_name, ':MSG_ID' => $row['MSG_ID'], ':SHARES_COUNT' => $row['SHARES_COUNT'], ':LIKES_COUNT' => $row['LIKES_COUNT'],
    ...
    }   // end of "foreach ($id_arry as $fan_group_name => $data)"
}
    ...
    foreach ($id_array as $fan_group_name => $data)
    {
        ...
        $update_sql = "UPDATE analysis_output AO
