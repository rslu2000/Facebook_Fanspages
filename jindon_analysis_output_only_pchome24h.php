<?php
    // http://localhost/xampp/jindon_analysis_output_only_pchome24h.php
    // 特別版本(以PCHOME24H的粉絲專頁做標準，分析全部貼文是否為業配文)
    ...
    $table_name = 'analysis_output_pchome24h';
...
if(1){  // 清除原本analysys_output，計算TF-IDF寫回資料表
    ...
    // 指定抓pchome24h粉絲專頁中TF-IDF高的資料
    $fan_group_name = 'pchome24h';
    
    // 將TF-IDF的結果排序
    // 排除特定詞類，長度為1的詞彙排除
    $sql = "SELECT CONCAT(WORD, '(', TYPE, ')') AS WORD_TYPE, pchome24h
              FROM `message_tfidf` 
             WHERE pchome24h > 0 
               AND TYPE NOT IN ('ADV','T','DET','P','M','ASP','POST','C')
